...
@Entity
@Table(name = "tipo_visualizacao")
public class VisualizationTypeModel {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "id_tipo_visualizacao")
    private Long id;

    @Column(name = "descricao", length = 50)
    private String description;

    @OneToMany(mappedBy = "visualizationType")
    private List<SignalizationDisplayLogModel> signalizationDisplayLogs;

    public VisualizationTypeModel() {}

    public VisualizationTypeModel(Long id, String description) {
        this.id = id;
